...
window.addEventListener('message', (e) => {
  if (e.data && e.data.type === 'SET_NICKNAME') {
    playerNickname = e.data.nickname
  } else if (e.data && e.data.type === 'LEADERBOARD_UPDATE') {
    const { scores, playerRank } = e.data.data
    renderLeaderboard(scores, playerRank)
  }
})
...
function endGame() {
  ...
  // Display 'Loading highscores...'
  const leaderboard = document.getElementById('leaderboard')
  const leaderboardList = document.getElementById('leaderboard-list')
  if (leaderboardList)
    leaderboardList.innerHTML =
      '<li style="color: var(--ctp-text);">Loading highscores...</li>'
  if (leaderboard) leaderboard.style.display = 'block'

  // Send score to React parent for saving (on exit) & getting leaderboard back
  window.parent.postMessage(
    { type: 'GAME_OVER', score: score, nickname: playerNickname },
    '*',
  )
...
function applyMovement() {
  // Horizontal: last-pressed wins when both held; 0 if neither
  ...
  // Vertical: last-pressed wins when both held; 0 if neither
  if (keysHeld.has('KeyW') && keysHeld.has('KeyS')) {
    player.dy = 0
  } else if (keysHeld.has('KeyW')) {
    player.dy = -1
  } else if (keysHeld.has('KeyS')) {
    player.dy = 1
  } else {
    player.dy = 0
  }
...
function renderLeaderboard(scores, playerRank) {
  const leaderboardList = document.getElementById('leaderboard-list')
  ...
  // Render the top 5
  tempScores.forEach((s, index) => {
    const isPlayer = s.nickname === playerNickname && playerRank === index + 1
    if (isPlayer) userInTop5 = true

    const color = isPlayer ? 'var(--ctp-peach)' : 'var(--ctp-text)'
    const el = document.createElement('li')
    el.style.color = color
    el.innerHTML = `<span>${index + 1}. ${s.nickname}</span> <span style="float: right;">${s.score} pts</span>`
    leaderboardList.appendChild(el)
  })

  // If the user didn't make the top 5, append them at their actual rank
  // Wait, if playerRank is null, something went wrong, just append at bottom
  if (!userInTop5) {
    if (tempScores.length === 5) {
      // Create a visual separator (optional)
      const sep = document.createElement('li')
      sep.innerHTML = `<span style="color: var(--ctp-surface1);">...</span>`
      leaderboardList.appendChild(sep)
    }

    let rankDisplay = playerRank || '?'
    const el = document.createElement('li')
    el.style.color = 'var(--ctp-peach)'
    el.innerHTML = `<span>${rankDisplay}. you (${playerNickname})</span> <span style="float: right;">${score} pts</span>`
    leaderboardList.appendChild(el)
  }
}
